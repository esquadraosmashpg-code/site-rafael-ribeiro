...
export function validateBookingPayload(payload) {
  ...
  // Aceite das condições comerciais (sinal, remarcação com 48h) --
  // separado do aceite de privacidade, exigido pouco antes da
  // confirmação (ver StepRevisao.js). Nunca confiar só na checagem do
  // frontend: revalidado aqui, no servidor.
  const aceiteCondicoesComerciais = payload?.aceiteCondicoesComerciais === true;
  ...
  if (!aceiteCondicoesComerciais) errors.push("É necessário concordar com as condições de agendamento.");

  return {
    valid: errors.length === 0,
    errors,
    value: { nome, email, whatsapp, modalidade, data, horario, aceitePrivacidade, aceiteCondicoesComerciais },
  };
}
